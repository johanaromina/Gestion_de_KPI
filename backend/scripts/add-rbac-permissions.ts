...
async function addRbacPermissions() {
  ...
  try {
    ...
    const scriptPath = join(__dirname, 'add-rbac-permissions.sql')
    const sqlScript = readFileSync(scriptPath, 'utf-8')

    console.log('🔧 Insertando permisos RBAC...')
    ...
    console.log('✅ Permisos RBAC insertados/verificados')
  } catch (error: any) {
