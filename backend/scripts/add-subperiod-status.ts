...
async function addSubPeriodStatus() {
  ...
  try {
    ...
    const scriptPath = join(__dirname, 'add-subperiod-status.sql')
    const sqlScript = readFileSync(scriptPath, 'utf-8')

    console.log('🔧 Ejecutando script de estado de subperiodos...')
    ...
    console.log('✅ Estado de subperiodos agregado/verificado')
  } catch (error: any) {
