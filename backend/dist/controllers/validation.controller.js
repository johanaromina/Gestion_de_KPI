...
export const validateConsistency = async (req, res) => {
    try {
        ...
        const [weightRows] = await pool.query(`SELECT SUM(weight) as totalWeight, COUNT(*) as kpiCount
       ...
       WHERE collaboratorId = ? AND periodId = ?`, [collaboratorId, periodId]);
        ...
        const [macroRows] = await pool.query(`SELECT ck.id, ck.kpiId, k.name as kpiName, k.macroKPIId, 
              mk.name as macroKpiName
       FROM collaborator_kpis ck
       JOIN kpis k ON ck.kpiId = k.id
       LEFT JOIN kpis mk ON k.macroKPIId = mk.id
       WHERE ck.collaboratorId = ? AND ck.periodId = ? AND k.macroKPIId IS NOT NULL`, [collaboratorId, periodId]);
        if (Array.isArray(macroRows) && macroRows.length > 0) {
            for (const row of macroRows) {
                // Verificar si el KPI macro también está asignado
                const [macroAssigned] = await pool.query(`SELECT id FROM collaborator_kpis
           WHERE collaboratorId = ? AND periodId = ? AND kpiId = ?`, [collaboratorId, periodId, row.macroKPIId]);
        ...
        const [unlinkedRows] = await pool.query(`SELECT ck.id, ck.kpiId, k.name as kpiName
       FROM collaborator_kpis ck
       JOIN kpis k ON ck.kpiId = k.id
       LEFT JOIN objective_trees_kpis otk ON k.id = otk.kpiId
       WHERE ck.collaboratorId = ? AND ck.periodId = ? 
       AND otk.kpiId IS NULL`, [collaboratorId, periodId]);
        ...
        const [saturationRows] = await pool.query(`SELECT COUNT(*) as kpiCount
       FROM collaborator_kpis
       WHERE collaboratorId = ? AND periodId = ?`, [collaboratorId, periodId]);
...
export const validatePeriodConsistency = async (req, res) => {
    try {
        const { periodId } = req.params;
        const [collaborators] = await pool.query(`SELECT DISTINCT collaboratorId 
       FROM collaborator_kpis 
       WHERE periodId = ?`, [periodId]);
        ...
        if (Array.isArray(collaborators) && collaborators.length > 0) {
            for (const collab of collaborators) {
                const [weightRows] = await pool.query(`SELECT SUM(weight) as totalWeight, COUNT(*) as kpiCount
           FROM collaborator_kpis
           WHERE collaboratorId = ? AND periodId = ?`, [collab.collaboratorId, periodId]);
                if (Array.isArray(weightRows) && weightRows.length > 0) {
                    const totalWeight = parseFloat(weightRows[0].totalWeight || 0);
                    if (Math.abs(totalWeight - 100) > 0.01) {
                        const [collabInfo] = await pool.query('SELECT name FROM collaborators WHERE id = ?', [collab.collaboratorId]);
                        allIssues.push({
