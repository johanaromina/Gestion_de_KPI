...
function calculateStatistics(results) {
    ...
    const variance = results.reduce((sum, val) => sum + Math.pow(val - average, 2), 0) /
        results.length;
...
export const getAggregatedByDirection = async (req, res) => {
    try {
        ...
        const [areas] = await pool.query(`SELECT DISTINCT area 
       FROM collaborators 
       ...
       ORDER BY area ASC`);
        ...
        for (const areaRow of areas || []) {
            ...
            const [collaborators] = await pool.query(`SELECT id, name, position, role 
         FROM collaborators 
         WHERE area = ?`, [area]);
            const results = [];
           ...
           WHERE collaboratorId = ? AND periodId = ?`, [collab.id, periodId]);
                if (Array.isArray(kpis) &&
                    kpis.length > 0 &&
                    kpis[0].totalWeight > 0) {
                    const result = (parseFloat(kpis[0].totalWeightedResult || 0) /
...
export const getAggregatedByManagement = async (req, res) => {
    try {
        const { periodId } = req.query;
        if (!periodId) {
            return res.status(400).json({ error: 'periodId es requerido' });
        ...
        const [managers] = await pool.query(`SELECT id, name, position, area 
       FROM collaborators 
       WHERE role = 'manager'
       ORDER BY area, name ASC`);
        ...
        for (const manager of managers || []) {
            ...
            const [teamMembers] = await pool.query(`SELECT id, name, position, area 
         FROM collaborators 
         WHERE managerId = ?`, [manager.id]);
            const results = [];
            // Incluir también el resultado del manager
            const [managerKpis] = await pool.query(`SELECT 
          SUM(weightedResult) as totalWeightedResult,
          SUM(weight) as totalWeight
         FROM collaborator_kpis
         WHERE collaboratorId = ? AND periodId = ?`, [manager.id, periodId]);
            if (Array.isArray(managerKpis) &&
                managerKpis.length > 0 &&
                managerKpis[0].totalWeight > 0) {
                const result = (parseFloat(managerKpis[0].totalWeightedResult || 0) /
            ...
            SUM(weight) as totalWeight
           FROM collaborator_kpis
           WHERE collaboratorId = ? AND periodId = ?`, [member.id, periodId]);
...
export const getAggregatedByLeadership = async (req, res) => {
    try {
        const { periodId } = req.query;
        if (!periodId) {
            return res.status(400).json({ error: 'periodId es requerido' });
        ...
        // Obtener líderes
        const [leaders] = await pool.query(`SELECT id, name, position, area 
       FROM collaborators 
       WHERE role = 'leader'
       ORDER BY area, name ASC`);
        const aggregatedData = [];
        for (const leader of leaders || []) {
            ...
            const [teamMembers] = await pool.query(`SELECT id, name, position, area 
         FROM collaborators 
         WHERE managerId = ?`, [leader.id]);
            const results = [];
            // Incluir también el resultado del líder
            const [leaderKpis] = await pool.query(`SELECT 
          SUM(weightedResult) as totalWeightedResult,
          SUM(weight) as totalWeight
         FROM collaborator_kpis
         WHERE collaboratorId = ? AND periodId = ?`, [leader.id, periodId]);
            if (Array.isArray(leaderKpis) &&
                leaderKpis.length > 0 &&
                leaderKpis[0].totalWeight > 0) {
                const result = (parseFloat(leaderKpis[0].totalWeightedResult || 0) /
...
export const getAggregatedByArea = async (req, res) => {
    try {
        const { periodId } = req.query;
        if (!periodId) {
            return res.status(400).json({ error: 'periodId es requerido' });
...
//# sourceMappingURL=aggregated-views.controller.js.map