...
export const getObjectiveTrees = async (req, res) => {
    try {
        const [rows] = await pool.query(`SELECT ot.*, 
       ...
       ORDER BY ot.level, ot.name ASC`);
        ...
        const objectivesWithKPIs = await Promise.all(rows.map(async (row) => {
            const kpiIds = row.kpiIds ? row.kpiIds.split(',').map(Number) : [];
            let kpis = [];
            if (kpiIds.length > 0) {
                const placeholders = kpiIds.map(() => '?').join(',');
                const [kpiRows] = await pool.query(`SELECT * FROM kpis WHERE id IN (${placeholders})`, kpiIds);
                kpis = kpiRows || [];
            }
            return {
                id: row.id,
...
export const getObjectiveTreeById = async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await pool.query('SELECT * FROM objective_trees WHERE id = ?', [id]);
        if (Array.isArray(rows) && rows.length === 0) {
            return res.status(404).json({ error: 'Objetivo no encontrado' });
        }
        const objective = rows[0];
        // Obtener KPIs asociados
        const [kpiRows] = await pool.query(`SELECT k.* FROM kpis k
       INNER JOIN objective_trees_kpis otk ON k.id = otk.kpiId
       WHERE otk.objectiveTreeId = ?`, [id]);
        res.json({
            ...objective,
            kpis: kpiRows || [],
        });
...
export const createObjectiveTree = async (req, res) => {
    try {
        const { level, name, parentId, kpiIds } = req.body;
        if (!level || !name) {
            return res.status(400).json({ error: 'Faltan campos requeridos' });
        }
        const [result] = await pool.query(`INSERT INTO objective_trees (level, name, parentId) 
       VALUES (?, ?, ?)`, [level, name, parentId || null]);
        ...
        if (kpiIds && Array.isArray(kpiIds) && kpiIds.length > 0) {
            for (const kpiId of kpiIds) {
                await pool.query(`INSERT INTO objective_trees_kpis (objectiveTreeId, kpiId) 
           VALUES (?, ?)`, [objectiveId, kpiId]);
            }
        }
        res.status(201).json({
            id: objectiveId,
...
export const updateObjectiveTree = async (req, res) => {
    try {
        const { id } = req.params;
        const { level, name, parentId, kpiIds } = req.body;
        await pool.query(`UPDATE objective_trees 
       SET level = ?, name = ?, parentId = ? 
       WHERE id = ?`, [level, name, parentId || null, id]);
        // Actualizar KPIs asociados
        // Primero eliminar todas las relaciones existentes
        await pool.query('DELETE FROM objective_trees_kpis WHERE objectiveTreeId = ?', [id]);
...
export const deleteObjectiveTree = async (req, res) => {
    try {
        const { id } = req.params;
...
//# sourceMappingURL=objective-trees.controller.js.map