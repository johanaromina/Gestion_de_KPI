 ...
 */
export const exportParrillaPDF = async (req, res) => {
    try {
      ...
      ORDER BY ck.createdAt ASC`, [collaboratorId, periodId]);
        const kpis = Array.isArray(kpiRows) ? kpiRows : [];
        // Crear documento PDF
        const doc = new PDFDocument({ margin: 50 });
        // Configurar headers de respuesta
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="parrilla_${collaborator.name}_${period.name}.pdf"`);
        ...
        doc.text(`Fecha: ${new Date(period.startDate).toLocaleDateString('es-ES')} - ${new Date(period.endDate).toLocaleDateString('es-ES')}`, { align: 'left' });
        doc.moveDown(2);
        // Tabla de KPIs
        if (kpis.length > 0) {
            ...
            kpis.forEach((kpi) => {
                ...
                doc.text(kpi.actual !== null ? parseFloat(kpi.actual).toFixed(2) : '-', tableLeft + colWidths[0] + colWidths[1], currentY);
                doc.text(`${parseFloat(kpi.weight).toFixed(2)}%`, tableLeft + colWidths[0] + colWidths[1] + colWidths[2], currentY);
                doc.text(kpi.variation !== null ? `${parseFloat(kpi.variation).toFixed(2)}%` : '-', tableLeft + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3], currentY);
                doc.text(kpi.weightedResult !== null
                    ? parseFloat(kpi.weightedResult).toFixed(2)
                    : '-', tableLeft + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4], currentY);
            ...
            const totalWeightedResult = kpis.reduce((sum, kpi) => sum + (parseFloat(kpi.weightedResult || 0)), 0);
            currentY += 10;
 ...
 */
export const exportParrillaExcel = async (req, res) => {
    try {
        const { collaboratorId, periodId } = req.params;
        if (!collaboratorId || !periodId) {
            return res.status(400).json({ error: 'collaboratorId y periodId son requeridos' });
      ...
      ORDER BY ck.createdAt ASC`, [collaboratorId, periodId]);
        ...
        res.setHeader('Content-Disposition', `attachment; filename="parrilla_${collaborator.name}_${period.name}.xlsx"`);
