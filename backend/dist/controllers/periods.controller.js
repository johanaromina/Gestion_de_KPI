...
export const getPeriods = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM periods ORDER BY startDate DESC');
        res.json(rows);
    }
    catch (error) {
...
export const getPeriodById = async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await pool.query('SELECT * FROM periods WHERE id = ?', [id]);
...
export const getSubPeriodsByPeriod = async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await pool.query('SELECT * FROM sub_periods WHERE periodId = ? ORDER BY startDate ASC', [id]);
        res.json(rows);
    }
    catch (error) {
...
export const createPeriod = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO periods (name, startDate, endDate, status) 
       VALUES (?, ?, ?, ?)`, [name, startDate, endDate, status || 'open']);
...
export const updatePeriod = async (req, res) => {
    try {
        const { id } = req.params;
        const { name, startDate, endDate, status } = req.body;
        await pool.query(`UPDATE periods 
       SET name = ?, startDate = ?, endDate = ?, status = ? 
       WHERE id = ?`, [name, startDate, endDate, status, id]);
...
export const closePeriod = async (req, res) => {
    try {
        ...
        const [periodRows] = await pool.query('SELECT * FROM periods WHERE id = ?', [id]);
